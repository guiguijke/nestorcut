...
impl Transformation {
    ...
    pub fn decompose(&self) -> DTransformation {
        let m = self.matrix();
        // SPIKE: libm — identical implementation on every target (std defers
        // to the platform libm: msvcrt natively vs Rust libm on wasm32, which
        // diverges by ulps and breaks replay determinism).
        let angle = libm::atan2f(m[1][0].into_inner(), m[0][0].into_inner());
