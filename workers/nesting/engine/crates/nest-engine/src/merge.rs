...
fn merge_bp_json(
    ...
    if config.finish_enabled() && bp_runs.iter().any(|r| r.finish.is_none()) {
        // §12.3 : la garde de la finition mesure désormais les distances
        // elle-même, sur les anneaux d'origine — plus de carte de collision,
        // donc plus d'instance jagua à importer ici. C'est autant de travail
        // en moins dans le thread de fusion du navigateur.
