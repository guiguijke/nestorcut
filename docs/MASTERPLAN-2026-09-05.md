...
| **Écart sous l'espacement de la tôle DENSE (1,883 mm)** | Localisé le 10/09 par `measure_svg_gaps.py` : paire hôte/pièce nichée sur la tôle que la finition ne touche pas ; candidats = solve sur anneaux simplifiés (borne ~1,90, insuffisante) ou post-pass hole-fill (repli de tolérance 1,90, insuffisant lui aussi). Mesure discriminante (poses avant/après post-pass) chiffrée à une demi-journée | **À trancher par l'owner** — non engagé (§16.3) |
