...
| **Écart sous l'espacement de la tôle DENSE (1,883 mm)** | **Deux défauts distincts (11/09, §18)** : (1) le hole-fill navigateur validait avec une distance sommet→segment (4,25 mm là où shapely mesure 0) → recouvrements réels et un job sur douze remboursé : **corrigé, miroir exact des deux langues, GO déploiement (§19)** ; (2) un résiduel 1 sur 24 à 1,8867 mm que le moteur livre tel quel : cause candidate = **embouchure du canal capillaire** (fenêtre 1,70-2,0 mm devant l'embouchure, §19.2), à vérifier sur le dump puis garde d'embouchure dans le moteur (§19.3) | **11/09 : (1) DÉPLOYÉ** (`e680537f`, §20 — corpus 11/11 sur l'image publiée, prod et homelab au même digest `68552630…`, moteur inchangé donc aucun benchmark à régénérer, app-ci vert). **(2) CONFIRMÉ par la mesure** (§19.2) : 1,8867 mm sur l'anneau brut, **2,0013 mm sur le polygone que le moteur reçoit** (canal de 2,1 mm), point le plus proche à **0,0 mm de la ligne d'embouchure** — **garde d'embouchure LIVRÉE le 11/09** (`af961aef`, §21) : les anneaux de trou bruts voyagent dans `EngineConfig.raw_holes`, le moteur les mesure à l'export et dégage ou retire la pièce nichée. Démo 8 × 3 : **24/24 à 2,000 mm**, corpus 11/11 avec neuf densités sur dix identiques, L2 inchangé. **DÉPLOYÉE le 11/09** (`91844704`, §23 : corpus 11/11 sur l'image publiée dont le binaire a été vérifié porteur de la garde, **dix densités publiques identiques**, worker + app + wasm dans la même fenêtre, prod et homelab au même digest `b3e1f4a4…`, wasm servi = dépôt). **PRIORITÉ 1 CLOSE** : le hole-fill ne crée plus de recouvrement, l'embouchure est gardée, et le moteur mesure lui-même ce qu'il livre |
| **Le tag `:latest` de ghcr suit `main`, pas les GO** (constat implémenteur 13/09) | Après le déploiement de S/E2 épinglé sur `53f93d2c`, `:latest` avait déjà avancé sur J1 + 2d, sans GO : un `compose pull && up -d` nu (prod OU homelab, dont la procédure §6 d'AGENTS fait exactement cela) déploierait du code non approuvé | **Arbitrage vérificateur (délégué)** : `:latest` ne bouge plus à chaque poussée. La CI publie `:<sha>` et `:main` ; un job manuel « promote » (workflow_dispatch, entrée = SHA approuvé) déplace `:latest`. Prod et homelab restent sur `:latest`, la procédure homelab redevient sûre. Lot **D1 livré `c2a619e3` et vérifié le 13/09 : en vigueur** (`:latest` ≡ prod, `compose pull && up -d` sans effet) ; en attendant, tout déploiement se fait par SHA (pull `:<sha>`, retag local, `up -d`) — écrit dans AGENTS §6 |
| **Changement d'implémenteur (13/09)** | Passation écrite : `docs/REPRISE-2026-09-13.md` (état prod, clos, en cours, J4, règles, outils) ; premier geste du nouvel agent : restaurer 86 fichiers QA retirés du suivi par `f95582d7` | Propriétaire 13/09 |
| **Vue DXF d'un résultat local : page figée (14/09)** | Constat owner en prod ; reproduit et tracé au débogueur : `uniquifyDxfHandles` (ligne à ligne) prend la couleur `62`/`5` du calque pour un code de handle et écrase le `0` du `ENDTAB` ⇒ boucle infinie de dxf-viewer. Régression depuis le lot 2b (12/09), tous les résultats « cet appareil ». Correctif H1 par paires (code, valeur) + vue DXF au harnais : `PLAN-CORRECTIF-VUE-DXF-2026-09-14.md` | Vérificateur 14/09 ; priorité 1 owner |
| **`.job` SheetCam, lot J4 (13/09)** | Livré par le nouvel implémenteur (`a139e9f8`…`c3b13b33`) : dépôt par signature, réglages pré-remplis, morsure d'amorce des trous (verdict de la recette traité), un `.job` par tôle, harnais navigateur. **Vérifié le 13/09 : NO-GO déploiement** — le défaut « 1 sur 2 » a sa cause dans `reduceForSolve` (forme J-085 qui jette les poses hors centre du planificateur ; reproduit : 1 hôte + 4 éventails ⇒ 4 sur 5, harnais vert) ; miroir Python identique (latent en prod pour un trou non circulaire) ; consigne J4-bis dans l'étude §9.28 ; recette machine : deux fichiers remis au propriétaire **J4-bis `ddaa7b0d` (forme compressée seulement si elle porte le plan, garde anti-perte des deux côtés, morsure à amorce nulle, harnais refait) vérifié le 13/09 soir : GO déploiement app + worker nesting**, conditionné au verdict machine du propriétaire ; avec la table des coins non confirmée, un seul éventail se niche au lieu de quatre (prix mesuré). Décision owner 13/09 : lot J4-ter = amorces reconstruites depuis le `.job`, calées sur une série de G-codes fournie par lui (étude §9.35), le `.job` seul suffisant en production. **Recette J4-bis : OK** sur l'amorce du trou (capture owner 13/09). Décisions owner du 13/09 soir : option « autoriser le chevauchement des amorces » (défaut non) ; **espacement `.job` = 2 × kerf + sécurité (1 mm par défaut)**, remplace la règle 3.10, à livrer (J4-bis-2) avant le déploiement. Série de rétro-ingénierie : règles d'arc mesurées et **points de départ lisibles et inscriptibles dans le bloc binaire** (étude §9.38) — NestorCut imposera le point de départ après nesting. **Série complète (19 h 30, étude §9.42) : la formule de pose du lot J4 est FAUSSE (45 mm à 45°, celle de J2 était juste) → NO-GO jusqu'au correctif ; `Start position` est l'origine de l'ordre de coupe, pas le départ des contours ; le vrai départ est dans le binaire → réserve au point lu (J4-bis-2 révisé, cinq points)**. **14/09 : le `.job` porte la géométrie des contours (segments décodés, étude §9.61) ; décision owner « on dépose le `.job` sans son DXF » ⇒ lot J6 « `.job` seul » avant la levée du gel (§9.62).** **Nuit du 13/09** : J4-bis-2/3 livrés et vérifiés ; G-code du propriétaire : SheetCam ne perce pas sur un POINT ; **le point de départ automatique du binaire est recalculé par SheetCam (deux fichiers à 90° du réel) ⇒ J4-ter, écrire le point nous-mêmes, conditionne le déploiement**. **J4-ter livré `2e4109a5` et vérifié : GO** ; déploiement `.job` (app + worker) dès la recette machine. **E3 livré `1d4ae531`/`35b27b0a` et vérifié : GO** après un cas serveur au harnais — priorité 3 refermée à son déploiement. **J4-ter + E3 DÉPLOYÉS le 14/09 à `9b227ec1`** (contrôle vérificateur : page à ce SHA, route E3 en place, homelab HEAD, benchmarks inchangés). Priorité 3 close ; priorité 4 : J4-quater (meilleur point de départ, §9.58) puis J5. **Accord owner 13/09 : la pièce L de test, deux `.job` (« none default », « + 45deg ») et leurs `.nc` entrent dans le dépôt comme fixtures** (`app/tests/fixtures/sheetcam/`) — dessin créé pour la série, aucune donnée client | Owner 13/09 ; vérificateur 13/09  **M1-ter VÉRIFIÉ le 24/09 : GO publication V0.9.7 — le verrou du clignotement exécute le plugin et échoue sur les deux versions fautives (éprouvé) ; application seule (plan des langues).** **M1-bis VÉRIFIÉ le 24/09 : NO-GO étroit — entrée de titre jamais retirée (le titre se fige après la première notification), test du clignotement qui n'exécute pas le plugin ; M1-ter de trois lignes (plan des langues).** **Lot M1 VÉRIFIÉ le 23/09 : NO-GO étroit — le titre de app.vue masque le clignotement de fin de calcul (régression éprouvée sur unhead), captures qui ne montrent pas le pied de page, une phrase anglaise restante ; M1-bis sur la même branche (plan des langues).** **Lot M1 ménage des langues OUVERT le 23/09 (décision owner) : titre d'onglet, pied de page public, lang pt-BR, guillemets en option — V0.9.7 ; puis revue de la feuille de route et choix du prochain chantier par le propriétaire (plan des langues).** **CHANTIER DES LANGUES CLOS le 23/09 — L4 espagnol publié (app `158ac627` V0.9.6, site `ab448ca`), contrôle conforme : six langues en ligne, neuf ancres espagnoles, digests identiques, correctif des dates prouvé dans le code servi, diagrammes portugais corrigés, branches supprimées. Chaque langue a réparé les précédentes (longueurs, licences, x-default, dates) ; neuf règles consignées (plan des langues).** **L4 paquet P PRÉPARÉ et contrôlé le 22/09 : GO aux trois écritures, sans correction préalable — verrou de source qui se prouve lui-même, 54 ancres résolues sur six langues, V0.9.6 à six blocs, aucun diff moteur. Répare aussi les dates pt/it/de et deux diagrammes portugais en production (plan des langues).** **L4 paquet C VÉRIFIÉ le 22/09 : GO — documentation espagnole complète et fidèle, dessins mesurés contre leur carte sur six langues (les deux diagrammes portugais de production corrigés, prêts à partir). Au paquet P : réparer le verrou de source (son motif du ternaire ne peut pas mordre), étendre le parseur du changelog à ES, générer et commiter la page Nouveautés espagnole, deux retouches, 54 ancres à résoudre. V0.9.6 (plan des langues).** **L4 révision A + paquet B VÉRIFIÉS le 22/09 : GO — dates réparées (intlTag, aucun ancien chemin), site espagnol juste (six sœurs, balise courte, huit traductions, article natif en brouillon). Corrections au paquet C : registre de connexion à refaire, verrou de source des dates, trois textes de diagramme hors de leur carte dont deux portugais en production (plan des langues).** **L4 espagnol paquet A (`d9372e6a`) VÉRIFIÉ le 22/09 : RÉVISION — dictionnaire bon (748/748, tutoiement tenu, glossaire, licences complètes d'emblée, 748 chaînes relues), mais la capture des réglages révèle que **cinq formatages de date ignorent la langue de l'application** (`quotaReset.js`, `PromoCodeSettings.vue`, `Subscription.vue`, `DeleteAccount.vue`, `ChatSupport.vue`) — pt, it et de affichent des dates anglaises en production ; plus l'espace du pourcentage espagnol (RAE), « Sus archivos » dans la suppression, deux `¡` manquants. Révision fusionnée avec le paquet B (plan des langues).** **L3 ALLEMAND PUBLIÉ le 22/09 (app `ecd205da` V0.9.5, site `1308c0f`) — contrôle conforme : cinq langues en ligne, neuf ancres allemandes vérifiées (umlaut compris), digests identiques, x-default réparé en production, zéro fragment orphelin, branches supprimées. Le cycle a réparé cinq défauts des langues précédentes (licences amputées en pt/it, résidu `}>Docs`, x-default, balises du sitemap, diagrammes anglais dans pt/it). L4 espagnol ouvert (plan langues).** **L3 paquet P PRÉPARÉ et contrôlé le 22/09 : GO aux trois écritures après restitution de la clause « ni une fuite » supprimée des diagrammes DE et PT par le raccourcissement (mesuré : elle tient dans le cadre à 800). Vérifié : 45 ancres d'aide résolues sur cinq langues, V0.9.5 à cinq blocs, vitest 814/814, aucun diff moteur, `:latest` encore sur l'italien, site bâti 154 pages sans fragment orphelin. Dette du vérificateur : la consigne « whatsNew au jour du déploiement » réarmait le badge Neu à chaque publication — une date de première production ne se réécrit jamais (plan des langues).** **L3 paquet C (`6e1473b` / app `dc46b64d`) VÉRIFIÉ le 22/09 : GO — documentation allemande complète (18 pages, structure identique au français, 19 images régénérées, espacement à 760 avec ses deux trajets), 15 diagrammes de blog traduits, et les six corrections vérifiées dans le HTML bâti (x-default, plan du site, fragments orphelins, commentaire). Cinq retouches au paquet P : diagramme de confidentialité coupé en DE et en PT (celui-ci en production), cinq captures du rapport à refaire après reconstruction, « außerhalb », « detailliert », ancres allemandes à umlauts. Paquet P = V0.9.5, cinq blocs de changelog (plan des langues).** **L3 révision A + paquet B (`5b0c4cde` / site `e66c7f7`) VÉRIFIÉS le 21/09 : GO, six corrections emportées dans le paquet C — diagrammes du blog en anglais dans de/it/pt (deux en production), `x-default` qui se désigne lui-même sur les accueils (chaîne vide falsy), sitemap en `de-DE`/`fr-FR` contre `de`/`fr` dans l'en-tête (exclut Autriche, Suisse, Belgique), article natif qui promet la doc, singulier du badge, « vendortierten ». Hotfix production `e715c07` (résidu `}>Docs` depuis L2) ratifié mais poussé sans demander et porteur d'un bout de paquet B ; son commentaire allemand est servi sur 63 pages (plan des langues).** **L3 allemand paquet A (`40eb9158`) VÉRIFIÉ le 21/09 : RÉVISION — dictionnaire bon (747/747, vouvoiement tenu, mots composés sans débordement, réglages et tableau des plans sondés en plus des captures), mais la page des licences est tronquée en DE **et en PT/IT déjà publiés** (usage commercial, MIT nest2d, sparrow/jagua-rs, marques), deux fautes « Sauberes Restblech » visibles dans la capture livrée, un diagnostic qui change de sens, l'espace du pourcentage allemand ; révision fusionnée avec le paquet B (plan des langues).** **L2 ITALIEN PUBLIÉ le 20/09 (app `88769cad` V0.9.4, site `7d73061`) — contrôle conforme : quatre langues en ligne, neuf ancres d'aide vérifiées, branches supprimées. L3 allemand ouvert (plan langues).** **L1 PORTUGAIS PUBLIÉ le 19/09 (app `9f4255c3` V0.9.3, site `d9778f6`) — contrôle conforme : trois surfaces en portugais, neuf ancres d'aide vérifiées en ligne, digests identiques, squash sans binaire. L2 italien ouvert (plan langues).** **L1 portugais, jalon A VÉRIFIÉ GO sous réserve d'A-bis (17/09) : application entière en portugais d'atelier ; verrou des variables à ajouter au test de parité ; aires m² via Intl ; jalon B (site + blog) s'ouvre.** **PROD = V0.9.2 (`12e3466e`, 16/09, conforme). Documentation : chantier CLOS (D1–D5). Ouverture de L1 portugais.** **D5-bis VÉRIFIÉ GO (16/09) : déploiement app V0.9.2 autorisé, puis L1 portugais.** **DOC COMPLÈTE PUBLIÉE (16/09, contrôle conforme). D5 VÉRIFIÉ : juste, trois retouches d'écran (D5-bis) puis GO/déploiement, puis L1 portugais.** **Documentation complète (8 sections) : GO DE FUSION D4 le 16/09 après relecture owner de Confidentialité (PR #15 `585663a`) ; publication puis D5 puis L1.** **PROD = V0.9.1 (`949f4f5e`, 16/09, contrôle conforme) ; D4-bis VÉRIFIÉ GO technique, fusion PR #15 après relecture owner de Confidentialité, puis D5, puis L1 portugais.** **L0-bis VÉRIFIÉ GO (16/09) : déploiement L0+L0-bis autorisé (V0.9.1) ; glossaires validés avec corrections ; D4 (PR #15) VÉRIFIÉ : Confidentialité conforme, neuf corrections ⇒ D4-bis puis relecture owner puis fusion (plans langues et documentation).** **Docs sections 1 à 4 PUBLIÉES (16/09, nestorcut.com/docs, contrôle conforme) ; L0 `0330795b` VÉRIFIÉ GO + L0-bis (html lang, endpoint mort, salut UTC) avant L1 (plan langues).** **Décision owner 16/09 : quatre langues de plus — portugais (Brésil), italien, allemand, espagnol ; polonais écarté — plan `docs/PLAN-LANGUES-2026-09-16.md` (socle L0 en parallèle de D4, puis une langue à la fois, complète : app + site + doc + captures, glossaire d'atelier et relecteur natif obligatoires).** **Documentation sections 1 à 4 : GO DE FUSION le 16/09 après relecture owner (PR #14 `77fcbce`), publication nestorcut.com/docs ; D4 ensuite.** **D3-bis `df1beb7` VÉRIFIÉ le 16/09 : GO technique publication des sections 1 à 4, sous relecture owner (page espacement d'abord) + un attribut du dessin FR (légende coupée) ; puis fusion PR #14 et contrôle en ligne (plan documentation).** **D3 documentation (même PR, `444e64f`) VÉRIFIÉ le 16/09 : sections 1 à 4 complètes et justes, menu prouvé, captures par langue ; cinq corrections courtes (dessin de l'espacement faux, exports verrouillés en gratuit, « Annuler », condition de la bande, « du trade ») ⇒ D3-bis, relecture owner, puis GO et publication (plan documentation).** **D2 documentation (même PR, `3b6948e`) VÉRIFIÉ le 16/09 : contenu solide, captures propres, neuf corrections absorbées dans D3 (menu doublé, un sens en gratuit, contour ouvert, DWG en erreur, captures FR sur les pages EN…) ; publication après les sections 1 à 4 (plan documentation).** **D1 documentation (PR #14 site vitrine) VÉRIFIÉ le 16/09 : socle GO (Starlight, FR/EN, recherche, build rejoué), publication NO-GO — cinq affirmations à corriger dont « Nos serveurs » faux en gratuit, captures du compte de dev à refaire sur un compte neuf, menu par section ⇒ D1-bis puis relecture owner (plan documentation).** **J11-ter DÉPLOYÉ le 16/09 à `a22b7d5a` (app seule) — contrôle conforme : SHA en page, V0.9.0, digests identiques, aucun diff moteur (plan UX §8). Prod = V0.9.0.** **J11-ter `bc5eca67` VÉRIFIÉ le 16/09 : GO app seule — vue agrandie `.job` réparée et regardée (amorces dessinées, badge, légende), un caractère de CHANGELOG corrigé par le vérificateur, SHA à promouvoir = commit docs + caractère ; dates `whatsNew.js` à remettre à la date du déploiement (plan UX §7).** **J11-bis `75758c4d` VÉRIFIÉ le 16/09 : NO-GO étroit — neuf points sur dix tiennent (carte groupée, Nouveautés, badges, modal, version, langue serveur), la vue agrandie d'une fiche `.job` est cassée à l'écran (légende sur le texte, aucune amorce dessinée) ⇒ J11-ter d'un écran, réparer ou revenir (plan UX §5).**  (plan UX §4) : carte groupée cassée à l'écran, page Nouveautés tronquée et en Markdown brut, badge « Nouveau » posé nulle part, slug encore au modal, version absente du pied de page de l'app ; règle : un lot d'interface se rapporte avec ses captures. J11-bis consigné.** , `docs/PLAN-DOCUMENTATION-2026-09-15.md`) : Starlight dans le site vitrine, FR + EN, recherche, huit sections, lots D1-D5 ; et décisions owner : prod = V0.9, CHANGELOG dédié, « on éclate par défaut » confirmé.** **Lot J11 ouvert (15/09, `docs/PLAN-UX-JOB-VERSION-2026-09-15.md`) : audit UI/UX du flux `.job` (projet nommé « 4) », carte groupée pour un `.job` multi-pièces, deux densités, vue agrandie sans amorces) + version affichée + badges « Nouveau ».**  (étude §9.82) : la chaîne `.job` est complète en production ; il ne reste que la recette du propriétaire, la liste d'essais du format et P4-10.**  : GO (étude §9.81), zéro verrou rouge.**  : GO app seule + J9-bis obligatoire (étude §9.79) — le fichier owner à quatre sections originales marche (4 fiches, 4 points réels), vitest sort en code 0.** , exigence owner « import/export parfaits ») : aller-retour 54/54 identique à l'octet (socle acquis), 38 champs binaires dont 19 inconnus mais 2 seulement variables ; plan en trois marches.**  : un `.job` du propriétaire à QUATRE sections originales du même dessin, chacune son point de départ — le binaire est indexé par SECTION ORIGINALE et non par nom de dessin, avec une sentinelle d'origine à reprendre du bloc précédent. Aujourd'hui : refusé seul, points perdus avec son DXF, `.job` rendu à 7 sections pour 4 pièces. Correctif : une fiche par section originale. **Lot J8 livré `c2b0fc23` puis VÉRIFIÉ NO-GO le 15/09 (étude §9.75) : J8-bis court demandé — l'écran se contredit (carte « .job acceptés » vs dépôt « non supporté »), vitest sort en code 1, un verrou de harnais rouge rapporté vert.**  : le `.job` redevient la sortie principale quand on en a déposé un (+ « tout télécharger » multi-tôles), et les amorces s'affichent dans la liste des fiches et la vue live. App seule, aucun impact calcul. **Lot J7 `efa0e37e` VÉRIFIÉ le 15/09 : GO app + worker nesting + homelab (étude §9.71), avec J6-ter.**  : la série « points de départ » du propriétaire est mesurée (6/6 décodés, points manuels intouchés à l'octet) et révèle un blocage antérieur — une PIÈCE SEULE de moins de 5 000 mm² sur une tôle 1000 × 1250 est refusée par le garde #2b, avec un conseil inapplicable. J7-a correctif côté payload (pas de moteur), J7-b verrous. **J6-ter `3256f706` vérifié (15/09) : GO app seule, non déployé (étude §9.68 ; rectificatif de ma mesure du §9.66).** **J6 `dad20367` puis J6-bis `777b48a8` vérifiés (14-15/09) : GO, déployés app seule — la réserve du doublon est levée (étude §9.66)** ; ancien libellé : (décodage indépendant 70 couples, harnais 4 passages ; réserve J6-bis : un DXF déposé après coup doit remplacer la fiche issue du binaire, étude §9.64). |
| **Pipeline « strip »** | Variante « bande » historique sans source dans le dépôt (images préconstruites), hors de portée de toutes les règles récentes (nettoyage E1 compris) | **Tranché par l'owner le 13/09 : retiré.** Consigne `docs/PLAN-RETRAIT-STRIP-2026-09-13.md`, lot S après E1-bis, données conservées, purge maintenue. **Lot S livré le 13/09** (rapport §3) : 23 fichiers supprimés, 49 modifiés — deux services compose, les routes `/strip` et `/api/strip/**`, les pages, le domaine serveur, le drapeau `isStripFeatureEnable` et les compteurs admin ; collections et buckets `strip_*` INTACTS, purge 24 h maintenue et verrouillée (`purge.test.js`). Deux arbitrages consignés : le panneau du coffre n'est plus derrière le drapeau strip (il devient visible pour tous les comptes — le serveur l'ouvrait déjà, AGENTS §35), et la ligne de partage requête/cycle de vie (job-dek nettoyé, purge et suppression de compte gardées) |
| **Tôle ou surface « sur photo »** (idée owner 12/09, non rangée) | Photographier une tôle ou une surface biscornue, poser une référence de dimension connue, cliquer le contour, en déduire la géométrie réelle et la garder en bibliothèque. Analyse vérificateur : (a) une seule longueur de référence ne corrige que l'échelle, pas la perspective — un rectangle connu (4 coins : feuille A4, gabarit 1000 mm) donne une homographie et une vraie métrique plane, précision attendue 2-5 mm sur 2 m avec un pointage à la main ; (b) **prérequis structurel : le conteneur polygonal** — aujourd'hui les tôles sont des rectangles de bout en bout (`sheets[].width/height`, `bin_width/bin_height`, post-pass, vérification, exports) alors que jagua accepte un conteneur polygonal ; ce prérequis est commun à la bibliothèque de chutes (priorité 6) et aux contraintes de tôle (priorité 11) | À ranger par l'owner ; proposition : chantier « conteneur polygonal » en tête, puis traceur photo + bibliothèque de surfaces avec la priorité 6 |
| **Pièces unitaires : éclatement + échelle à l'import, traits fins** | Demande d'un atelier réel (logos complets à nester pièce par pièce, mise à l'échelle) ; le fichier fourni faisait tomber le moteur : gonflement jagua non robuste sur un ruban de 0,18 mm (diagnostic `docs/PLAN-ECLATEMENT-2026-09-12.md` §1) ; consigne E0 moteur + message, E1 import avancé navigateur, E2 miroir serveur | **Tranché par l'owner le 12/09 : priorité 1, avant le `.job`**. **E0 livré `b1da2690`, vérifié le 12/09 : GO déploiement** — repli de gonflement (Minkowski entier) sur échec de `geo_buffer`, chemin actuel bit-identique (`a1bd8810…`), 205 fichiers au moteur : 13 → 4 refus d'import, 0 panique ; message qui nomme fichier et pièce ; E1 (import avancé) ensuite **E0 déployé `28880d01`** (wasm prod = dépôt, homelab HEAD, benchmarks sans écart). **E1 livré `0ec498d0`, vérifié le 12/09 : GO** — panneau « Import avancé » replié et éteint, éclatement 17 fiches / 17 posées, échelle ×0,5 et largeur cible exactes, micro-vides rebouchés des deux côtés, moteur × 238 : 0 refus ; **E1 déployé `76738086`** ; **E1-bis (aperçu sur une tôle, poignée, facteur affiché) livré `7ddef81f`, vérifié le 13/09 : GO** ; **E1-bis déployé `3db283c2`** ; **lot S (retrait strip) `3824d84c` et E2 (miroir serveur) `241f5dda` vérifiés le 13/09 : GO** — après leur déploiement la priorité 3 est close ; 704 entités non attachées sur 9 fichiers → couture (priorité 5) ; importeur serveur non reproductible sur un fichier pathologique (défaut préexistant, à traiter avec la couture). **E2 livré le 13/09** (rapport §5) : miroir serveur complet — échelle (facteur et largeur/hauteur cible, résolue sur l'étendue mesurée), éclatement par sous-ensemble de handles en fiches normales, constat « pièces plus fines que l'espacement » du moteur jusqu'au rapport, nom du DXF résultat borné à trois noms ; **parité navigateur ≡ serveur mesurée sur le logo : 17 fiches, 0 écart** de pièces et d'encombrement. Au passage, un DÉFAUT DE PRODUCTION corrigé : une entité qui touche une pièce sans la traverser n'était attachée à rien, donc ABSENTE du DXF de coupe (720 → 704 entités orphelines sur le corpus, 143/148 fichiers inchangés). Écart de nettoyage restant entre les deux importeurs : 2 fichiers sur 137, même géométrie, compte d'ergots non mesurable côté serveur (grille shapely) — couture, priorité 5. **Lots S et E2 déployés le 13/09 à `d890c923`** (prod 404 sur les routes strip, homelab à HEAD, corpus 11/11, benchmarks régénérés : une seule fiche bouge, T-F dans sa bande d'oscillation). **Priorité 3 CLOSE** sur le fond — **rouverte le 13/09 soir sur l'ergonomie** : le propriétaire ne veut pas éclater par défaut ni ouvrir un panneau avant de déposer ; il veut un **interrupteur « Import avancé » à la création du projet** (bloc « New nesting », visible quand allumé) et, allumé, **une fenêtre de choix à chaque dépôt** : « import automatique » (défaut) ou « éclater + mettre à l'échelle » (aperçu E1-bis). Lot **E3**, consigne `PLAN-ECLATEMENT` §6, après J4-bis-2 et avant J4-ter, app seule  **E3 déployé le 14/09 puis jugé non conforme à l'usage : décision owner 14/09 — un dessin non éclaté se neste comme un BLOC RIGIDE, l'échelle et l'éclatement vivent sur la fiche, la fenêtre et l'interrupteur disparaissent ⇒ lot E4, consigne `PLAN-ECLATEMENT` §8** |
| **Réparations d'import (phase C, `docs/qa/import-2026-09-09/synthese.md` §4)** | R1 unités + R3 messages d'abord ; R2 couture des contours précédée d'un **inventaire GitHub** (½ j) : pistes connues `ezdxf.edgeminer` / `ezdxf.edgesmith` (≥ 1.4 : chaînage d'arêtes libres avec tolérance de gap, côté Python) et équivalents Rust pour le chemin wasm | **Tranché par l'owner le 10/09 : important, chercher la solution existante avant de coder** — priorité 2. **Lot 2a (garde) livré `7149fb56`, vérifié le 11/09 (`PLAN-IMPORT` §9.5) : GO** — 150/153 réels lus en navigateur (contre 140), 0 dérive, arbitrage vérificateur : budget de temps par implémentation, navigateur 20 s / serveur 60 s (dans le code, `cbacd63a`). **DÉPLOYÉ le 12/09** : app + wasm géométrie + worker fileprocessing dans la même fenêtre, digests `688d4fdd…` / `92ca2725…`, wasm servi = dépôt, bundle de prod vérifié sur les fichiers réels (1 841 et 6 144 entités lus, lourd refusé avec son nombre) ; 0 refus de temps côté serveur sur les 153 réels. **Lot 2b (unités) livré `52871f48`, vérifié le 12/09 : GO** — table 0-20 exacte des deux côtés, parité des unités 0 divergence sur 236 fichiers lus, 8/8 sans unité avec constat, exports CAM relus ; écart de l'exporteur CORRIGÉ (`c84b0842` : tous les codes entiers des entités et de la table des calques en entiers, verrou qui lit la sortie, exports-parity vert en CI). **DÉPLOYÉ le 12/09** : app + wasm géométrie + **les deux images worker** (`units.py` vit dans `worker_common`), digests `47f4dfca…` / `77a43274…` / `1836199b…`, table 20 codes active dans les deux workers de prod, wasm servi = dépôt, **homelab recréé et `ASSERT OVERFLOW=HEAD: OK`** ; puis 2c messages, 2d temps. **Lot 2d livre et deploye le 13/09** (`7622f46c`, worker fileprocessing seul) : les sondes d'attachement sortent de la boucle des empreintes — corpus 132,3 s -> 15,4 s, pire fichier 54,7 s -> 0,95 s, 148/148 fichiers identiques. **Lot 2e livre et deploye le 13/09** (`baf39cb7`, app + wasm geometrie) : index de boites pour le noding — navigateur 76,7 s -> 30,5 s, le fichier refuse a 20 s est lu en 7,9 s, parite golden 100 %, determinisme 68/68. **PRIORITE 2 CLOSE des deux cotes**, plus aucun fichier du corpus au-dela de 10 s. |
