...
| 5 | **Phase C import** : lots A-E **livrés le 10/09** (`1bc5eec7`, 85 DXF dont 4 réels, coureurs wasm et ezdxf, synthèse stratifiée, rapport de réparation spécifié). Reste : fichiers réels des entretiens dans `specs/import-corpus/` (owner), choix R1/R2/R3 (registre §5) | Owner (fichiers + choix) |
...
| **Garde de faisabilité de la finition** | Rejet sur le contour seul (borné à space − 0,05), paires en trace ; oracle exact en tranche 2 ; l'écart sous espacement sans recouvrement reste mesuré et affiché en aval (même exposition que tout job mono-tôle) | **Tranché 10/09** (vérificateur, arbitrage délégué) |
| **Réparations d'import (phase C, `docs/qa/import-2026-09-09/synthese.md` §4)** | R1 unités (2 j, ×10⁶ et ×25,4 silencieux, dont nos propres exports CAM) et R3 message + garde précoce (1,5 j) d'abord ; R2 couture des contours (4-6 j, risque déterminisme élevé) après un corpus réel | **À trancher par l'owner** — reco vérificateur 10/09 : R1 + R3, puis R2 |
