...
| **Pièces unitaires : éclatement + échelle à l'import, traits fins** | Demande d'un atelier réel (logos complets à nester pièce par pièce, mise à l'échelle) ; le fichier fourni faisait tomber le moteur : gonflement jagua non robuste sur un ruban de 0,18 mm (diagnostic `docs/PLAN-ECLATEMENT-2026-09-12.md` §1) ; consigne E0 moteur + message, E1 import avancé navigateur, E2 miroir serveur | **Tranché par l'owner le 12/09 : priorité 1, avant le `.job`**. **E0 livré `b1da2690`, vérifié le 12/09 : GO déploiement** — repli de gonflement (Minkowski entier) sur échec de `geo_buffer`, chemin actuel bit-identique (`a1bd8810…`), 205 fichiers au moteur : 13 → 4 refus d'import, 0 panique ; message qui nomme fichier et pièce ; E1 (import avancé) ensuite |
