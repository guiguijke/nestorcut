...
echo "=== E2E 0.1 ==="
QA_OUT="$T/e2e-l2ter-s01" QA_SPACE=0.1 node "$T/qa-e2e-freeze.mjs" > "$T/e2e-l2ter-s01.log" 2>&1
grep -E "compute outcome|LONGTASKS|FATAL|pageerror" "$T/e2e-l2ter-s01.log" | cut -c1-300
echo "=== E2E 2 ==="
QA_OUT="$T/e2e-l2ter-s2" QA_SPACE=2 node "$T/qa-e2e-freeze.mjs" > "$T/e2e-l2ter-s2.log" 2>&1
grep -E "compute outcome|LONGTASKS|FATAL|pageerror" "$T/e2e-l2ter-s2.log" | cut -c1-300
