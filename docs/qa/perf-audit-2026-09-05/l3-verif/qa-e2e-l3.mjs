...
try {
    ...
    // Lot 3 (B.4) : deux champs — kerf 0 + sécurité = space/2 ⇒ effectif = QA_SPACE
    const half = String(Number(process.env.QA_SPACE || '0.1') / 2)
    const kerfF = page.locator('label.input', { hasText: 'Kerf' }).locator('.input__value')
    await kerfF.click(); await kerfF.fill('0'); await kerfF.blur()
    const safetyF = page.locator('label.input', { hasText: 'Safety' }).locator('.input__value')
    await safetyF.click(); await safetyF.fill(half); await safetyF.blur()
    log('kerf/safety set: 0 /', half)
