...
try {
    ...
    // --- 4. deuxième calcul : le bouton Annuler doit être RÉARMÉ ---
    await page.screenshot({ path: "C:/Users/guiguijke/.claude/jobs/b6cdf1c7/tmp" + '/c01-before-renest.png' }).catch(() => {})
    const nestBtn = page.locator('.atelier__nest')
    log('nest button state before 2nd click: disabled =', await nestBtn.isDisabled().catch(() => null), 'box =', JSON.stringify(await nestBtn.boundingBox().catch(() => null)))
    try { await nestBtn.click({ timeout: 8000 }) } catch (e) { log('normal click blocked:', String(e.message).slice(0, 120), '-> force click'); await nestBtn.click({ force: true }) }
    log('nest clicked again')
    ...
    if (disabled2 || !/^cancel$/i.test(label2)) throw new Error('C01 REGRESSION: cancel button not re-armed on second run (label=' + label2 + ', disabled=' + disabled2 + ')')
    await btn2.click()
    let freed2 = false
    for (let i = 0; i < 15; i++) { await page.waitForTimeout(2000); if (!(await page.locator('.stage__status').count()) && !(await page.locator('[data-testid="live-cancel"]').count())) { freed2 = true; break } }
    log('second cancel freed:', freed2)
    if (!freed2) throw new Error('second cancel did not free the stage')
    console.log('C01 OK — annulation calcul navigateur depuis la vue live (x2)')
    await browser.close()
    process.exit(0)
} catch (e) {
