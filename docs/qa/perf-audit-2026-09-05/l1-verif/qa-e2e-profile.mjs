...
try {
    ...
    try { const { profile } = await cdp.send('Profiler.stop'); fs.writeFileSync(path.join(OUT, 'cpuprofile.json'), JSON.stringify(profile)); log('profile saved', profile.samples.length) } catch (e) { log('profile failed', String(e).slice(0, 200)) }
